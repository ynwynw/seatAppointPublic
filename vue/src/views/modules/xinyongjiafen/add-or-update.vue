...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if(obj!=null) {
	      obj.xinyongfen = parseFloat(obj.xinyongfen) + parseFloat(this.ruleForm.xinyongfen)
	      this.$http({
		  url: `${table}/update`,
		  method: "post",
		  data: obj
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xinyongjiafen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinyongjiafenCrossAddOrUpdateFlag = false;
                  this.parent.search();
