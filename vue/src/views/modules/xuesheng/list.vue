<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','信用分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="xinyongfenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信用分':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','信用分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="xinyongfenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信用分':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','信用分') && contents.tableBtnIcon == 0" type="success" size="mini" @click="xinyongfenCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'信用分':'' }}</el-button>
    ...
    <xinyongfen-cross-add-or-update v-if="xinyongfenCrossAddOrUpdateFlag" :parent="this" ref="xinyongfenCrossaddOrUpdate"></xinyongfen-cross-add-or-update>
...
import xinyongfenCrossAddOrUpdate from "../xinyongfen/add-or-update";
export default {
  data() {
    return {
      banjiOptions: [],
      ...
      xinyongfenCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    xinyongfenCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.xinyongfenCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
      this.$refs.xinyongfenCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
