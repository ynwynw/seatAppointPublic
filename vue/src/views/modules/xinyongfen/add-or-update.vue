<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="信用分" prop="xinyongfen">
          <el-select v-model="ruleForm.xinyongfen" placeholder="请选择信用分">
            <el-option
                v-for="(item,index) in xinyongfenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.xinyongfenOptions = "100".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xinyongfen/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xinyongfen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xinyongfenCrossAddOrUpdateFlag = false;
                  this.parent.search();
